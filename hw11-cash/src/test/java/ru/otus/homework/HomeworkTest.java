...
/*
    ...
    public void testHomeworkRequirementsForUpdatesCount() {
        ...
        var client = new Client(null, "Vasya", new Address(null, "AnyStreet"),
            ...
            var client1 = new Client(null, "Vasya", new Address(null, "AnyStreet"),
                    List.of(new Phone(null, "13-555-22"), new Phone(null, "14-666-333")));
            var client2 = new Client(null, "Vasya", new Address(null, "AnyStreet"),
                    List.of(new Phone(null, "13-555-22"), new Phone(null, "14-666-333")));
        try (var session = sessionFactory.openSession()) {
            ...
            session.clear();
            logger.info("До "+LocalDateTime.now().toString());
            var loadedClient2 = session.find(Client.class, 2L);
            var loadedClient3 = session.find(Client.class, 3L);
            var loadedClient4 = session.find(Client.class, 2L);
            var loadedClient = session.find(Client.class, 1L).clone();
            logger.info("После"+LocalDateTime.now().toString());
